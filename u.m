...
properties (Constant = true)
    ...
    lP = scd(u.h_bar/(u.mP*u.c),'lP') % Planck length
    ...
    HCF = scd(u.CCF,'HCF') % hundred cubic feet
    MCF = scd(1000*u.ft3,'MCF') % mille cubic feet
    MMCF = scd(1e6*u.ft3,'MMCF') % mille mille (1e6) cubic feet
    BCF = scd(1e9*u.ft3,'BCF') % billion cubic feet
    TMC = scd(u.BCF,'TMC') % thousand million cubic feet
    TCF = scd(1e12*u.ft3,'TCF') % trillion cubic feet
    ...
    Gg = scd(1e9*u.gram,'Gg') % Gigagram
    Gigagram = scd(u.Gg,'Gigagram') 
    ...
    amu = scd(1.66053906660e-27*u.kg,'amu') % atomic mass unit
    ...
    mP = scd(sqrt(u.h_bar*u.c/u.G),'mP') % Planck mass
    ...
    tP = scd(u.lP/u.c,'tP') % Planck time
    ...
    TJ = scd(1e12*u.J,'TJ') % terajoule
    terajoule = scd(u.TJ,'terajoule')
    ...
    eV = scd(u.e/u.C*u.J,'eV') % electronvolt
    ...
    kBtu = scd(1e3*u.Btu,'kBtu') % kiloBtu
    MMBtu = scd(1e6*u.Btu,'MMBtu') % million Btu
    britishThermalUnit = scd(u.Btu,'britishThermalUnit') 
    Btu_IT = scd(1055.05585*u.J,'Btu_IT') % British thermal unit (International Table)
    ...
    E_h = scd(u.alpha^2*u.m_e*u.c^2,'E_h') % Hartree energy
    ...
    Z0 = scd(2*u.h_c*u.alpha/u.e^2,'Z0') % characteristic impedance of vacuum
    impedanceOfVacuum = scd(u.Z0,'impedanceOfVacuum') 
    R_K = scd(u.h_c/u.e^2,'R_K') % von Klitzing constant
    ...
    sigma_SB = scd(pi^2/60*u.k^4/u.h_bar^3/u.c^2,'sigma_SB') % Stefan-Boltzmann constant
    Stefan_BoltzmannConstant = scd(u.sigma_SB,'Stefan_BoltzmannConstant') 
    mu_B = scd(u.e*u.h_bar/(2*u.m_e),'mu_B') % Bohr magneton
    BohrMagneton = scd(u.mu_B,'BohrMagneton') 
    mu_N = scd(u.e*u.h_bar/(2*u.m_p),'mu_N') % nuclear magneton
    nuclearMagneton = scd(u.mu_N,'nuclearMagneton') 
    ly = scd(u.c*u.year,'ly') % light-year
    lightYear = scd(u.ly,'lightYear') % light-year
    mu0 = scd(2*u.alpha*u.h_c/u.e^2/u.c,'mu0') % vacuum permeability
    ...
    K_J = scd(2*u.e/u.h_c,'K_J') % Josephson constant
    ...
    F_c = scd(u.e*u.N_A,'F_c') % Faraday constant
    FaradayConstant = scd(u.F_c,'FaradayConstant') 
    c1 = scd(2*pi*u.h_c*u.c^2,'c1') % first radiation constant
    firstRadiationConstant = scd(u.c1,'firstRadiationConstant') 
    c2 = scd(u.h_c*u.c/u.k,'c2') % second radiation constant
    secondRadiationConstant = scd(u.c2,'secondRadiationConstant') 
    b_prime = scd(2.821439372122078893*u.k/u.h_c,'b_prime') % Wien frequency displ. law const.
    WienFrequencyDisplacementLawConstant = scd(u.b_prime,'WienFrequencyDisplacementLawConstant') 
    b_c = scd(u.h_c*u.c/(4.965114231744276303*u.k),'b_c') % Wien wavelength displ. law const.
    ...
    R_bar = scd(u.N_A*u.k,'R_bar') % molar gas constant
